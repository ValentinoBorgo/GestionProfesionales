...
class EditUser extends EditRecord
{
    ...
    public function mount(string|int|null $record): void
    {
        parent::mount($record);
        ...
        if ($tipoPersonaProfesional && intval($this->record->id_tipo) === intval($tipoPersonaProfesional->id)) {
            ...
            if ($tipoPersonaProfesional && intval($this->record->id_tipo) === intval($tipoPersonaProfesional->id)) {
                ...
                foreach ($this->horariosGuardados as $horario) {
                    ...
                    if (isset($horario['id']) && !empty($horario['id'])) {
                        Disponibilidad::where('id', $horario['id'])->update([
    ...
    protected function loadExistingHorarios()
    {
        $datos = $this->form->getRawState();

        $disponibilidades = Disponibilidad::where('id_usuario', $this->record->id)->get();

        $this->horariosGuardados = $disponibilidades->map(function ($disponibilidad) {
            return [
                'id' => $disponibilidad->id,
                'dias_semana' => $disponibilidad->dia,
                'hora_entrada' => $disponibilidad->horario_inicio,
                'hora_salida' => $disponibilidad->horario_fin,
                'sala' => $disponibilidad->sala->nombre ?? 'Desconocida',
                'idSala' => $disponibilidad->id_sala,
            ];
        })->toArray();
    ...
    public function editarHorario($index)
    {
        $datos = $this->form->getRawState();
        if (!isset($this->horariosGuardados[$index])) {
            return;
        }
        $horario = $this->horariosGuardados[$index];
        $this->form->fill(array_merge($datos,[
    ...
    public function guardarHorario()
    {
        ...
        $idHorario = $this->editingIndex !== null && isset($this->horariosGuardados[$this->editingIndex]['id'])
            ? $this->horariosGuardados[$this->editingIndex]['id']
            : null;
    
        $nuevoHorario = [
            'id' => $idHorario,
            'dias_semana' => $this->form->getState()['dias_semana'],
            'hora_entrada' => $this->form->getState()['hora_entrada'],
            'hora_salida' => $this->form->getState()['hora_salida'],
        ...
        if ($this->editingIndex !== null) {
            ...
            $this->horariosGuardados[$this->editingIndex] = $nuevoHorario;
            $this->editingIndex = null; // Salir del modo edición
        } else {
    ...
    public function eliminarHorario($index)
    {
        ...
        if (isset($this->horariosGuardados[$index])) {
            $horarioId = $this->horariosGuardados[$index]['id'];
            DB::table('disponibilidad')->where('id', $horarioId)->delete();
